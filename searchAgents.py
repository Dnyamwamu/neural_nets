...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        # Define corners based on maze dimensions
        top = self.walls.height - 2
        right = self.walls.width - 2
        self.corners = ((1,1), (1,top), (right,1), (right,top))

        # Keep track of which corners exist in this maze
        self.corners = tuple(c for c in self.corners if not self.walls[c[0]][c[1]])

        self._expanded = 0  # For display/debugging

        # Start state: (position, visited_corners)
    ...
    def getSuccessors(self, state):
        ...
        # Possible moves
        for dx, dy, action in [(-1,0,'West'), (1,0,'East'), (0,1,'North'), (0,-1,'South')]:
            nextx, nexty = x + dx, y + dy

            if not self.walls[nextx][nexty]:
                nextPosition = (nextx, nexty)
                # If this is a corner, add it to visitedCorners
                if nextPosition in self.corners and nextPosition not in visitedCorners:
                    nextVisited = tuple(list(visitedCorners) + [nextPosition])
                else:
...
def cornersHeuristic(state, problem):
  ...
  if not unvisited:
    ...
    # Heuristic: distance to farthest unvisited corner
...
def foodHeuristic(state, problem):
  ...
  "*** YOUR CODE HERE ***"
  """
    Heuristic for FoodSearchProblem:
    Use the maze distance to the farthest food dot as the estimate.
  """
  position, foodGrid = state
  ...
  if not foodList:
    ...
    # Compute the maze distance to each food dot
